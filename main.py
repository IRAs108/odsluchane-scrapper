...
def bs_parse(content):
    ...
    try:
        ...
        for soupa in soup_base:
            if soupa.select('td.bg-gray') or soupa.select('div.googl-ad-responsive'):
                continue
            if soupa.select('td'):
                time = soupa.select('td')[0].getText()
                lista.append(time)
            if soupa.select('td a.title-link'):
                song = soupa.select('td a.title-link')[0].getText().strip()
                lista.append(song)
    except:
        logging('Problem z przetwarzaniem strony')
    return lista
...
def save_data(data, st, day, month, year):
    ...
    for dd in data:
        if len(dd) == 5:
            ...
            header = ['data', 'godzina', 'utwor', 'stacja']
...
while True:
    ...
    if len(odpowiedz) == 0:
        logging.warn("Brak danych dla wybranego okresu")
        continue
